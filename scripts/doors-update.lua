...
do
    ...
    if emoji then Title.Text = ("%s %s %s"):format(emoji, Title.Text, emoji) end
end
...
PluginsHint.Text =
    "Download plugins from the IY Discord (discord.io/infiniteyield)"
...
function dragGUI(gui)
    task.spawn(function()
        ...
        local function update(input)
            ...
            TweenService:Create(gui, TweenInfo.new(.20), {Position = Position})
                :Play()
        end
...
reference = (function()
    ...
    inviteButton.MouseButton1Click:Connect(function()
        ...
        inviteButton.Text =
            "Copy Discord Invite Link (https://discord.io/infiniteyield)"
    end)
    dragGUI(main)
    main.Parent = PARENT
        ...
        if pcall(function() readfile("IY_FE.iy") end) then
            if readfile("IY_FE.iy") ~= nil then
                local success, response = pcall(function()
                    ...
                    if json.keepIY ~= nil then
                        KeepInfYield = json.keepIY
                    else
                        KeepInfYield = true
                    end
                    if json.espTransparency ~= nil then
...
Clear_2.MouseButton1Down:Connect(function()
    for _, child in pairs(scroll_3:GetChildren()) do child:Destroy() end
    scroll_3.CanvasSize = UDim2.new(0, 0, 0, 10)
end)
...
task.spawn(function()
    coroutine.resume(coroutine.create(function()
        local ChatEvents = ReplicatedStorage:WaitForChild(
                               "DefaultChatSystemChatEvents", math.huge)
            ...
            OnMessageEvent.OnClientEvent:Connect(function(data)
                if data ~= nil then
                    ...
                    if string.find(originalchannel, "To ") then
                        message = "/w " ..
                                      string.gsub(originalchannel, "To ", "") ..
                                      " " .. message
                    end
...
Players.LocalPlayer.CharacterAdded:Connect(function()
    ...
    if not Clip then execCmd('clip') end
...
Cmdbar.Focused:Connect(function()
    ...
    tabComplete = UserInputService.InputBegan:Connect(function(input,
        ...
        if Cmdbar:IsFocused() then
            if tabAllowed == true and input.KeyCode == Enum.KeyCode.Tab and
                topCommand ~= nil then autoComplete(topCommand) end
        else
        ...
        for i, v in pairs(binds) do
            if not v.ISKEYUP then
                if (input.UserInputType == Enum.UserInputType.Keyboard and
                    v.KEY:lower() == tostring(input.KeyCode):lower()) or
                    (input.UserInputType == Enum.UserInputType.MouseButton1 and
                        v.KEY:lower() == 'leftclick') or
...
ClickTP.Select.MouseButton1Click:Connect(function()
    if keySelected then
        ...
        if keyPressed == 'RightClick' or keyPressed == 'LeftClick' then
            ...
            notify('Keybinds Updated',
                   'Binded ' .. keyPressed:sub(14) .. ' to click tp')
        end
    end
end)
...
ClickDelete.Select.MouseButton1Click:Connect(function()
    if keySelected then
        ...
        if keyPressed == 'RightClick' or keyPressed == 'LeftClick' then
            notify('Keybinds Updated',
                   'Binded ' .. keyPressed .. ' to click delete')
        else
            notify('Keybinds Updated',
                   'Binded ' .. keyPressed:sub(14) .. ' to click delete')
        end
    end
end)
...
local function clicktpFunc()
    pcall(function()
        ...
        local newCFrame = CFrame.new(hitPosition, Vector3.new(
                                         rootPartPosition.X, hitPosition.Y,
                                         rootPartPosition.Z)) *
...
function deletePlugin(name)
    local pName = name .. '.iy'
    if name:sub(-3) == '.iy' then pName = name end
    for i = #cmds, 1, -1 do
...
addcmd('jobid', {}, function(args, speaker)
    local jobId = 'Roblox.GameLauncher.joinGameInstance(' .. PlaceId .. ', "' ..
                      JobId .. '")'
    toClipboard(jobId)
end)
...
addcmd('gametp', {'gameteleport'},
       function(args, speaker) TeleportService:Teleport(args[1]) end)
        ...
        if body and body.data then
            for i, v in next, body.data do
                if type(v) == "table" and tonumber(v.playing) and
                    tonumber(v.maxPlayers) and v.playing < v.maxPlayers and v.id ~=
                    JobId then table.insert(servers, 1, v.id) end
            end
        end
        if #servers > 0 then
            TeleportService:TeleportToPlaceInstance(PlaceId,
                                                    servers[math.random(1,
                                                                        #servers)],
                                                    Players.LocalPlayer)
        else
        ...
        if not pcall(function()
            ...
            if not FoundUser then
                ...
                if GUID ~= nil then
                    ...
                    TeleportService:TeleportToPlaceInstance(Place, GUID,
                                                            Players.LocalPlayer)
                else
...
addcmd('tweenwaypoint', {'twp'}, function(args, speaker)
    ...
    if speaker.Character then
        for i, _ in pairs(WayPoints) do
            ...
            local z = WayPoints[i].COORD[3]
            if tostring(WayPoints[i].NAME):lower() == tostring(WPName):lower() then
                TweenService:Create(getRoot(speaker.Character), TweenInfo.new(
...
addcmd('cleargamewaypoints', {'cgamewp'}, function(args, speaker)
    for i, v in pairs(WayPoints) do
        if v.GAME == PlaceId then table.remove(WayPoints, i) end
    end
    if AllWaypoints ~= nil and #AllWaypoints > 0 then
        for i, v in pairs(AllWaypoints) do
            if v.GAME == PlaceId then table.remove(AllWaypoints, i) end
        end
    end
    for i, v in pairs(pWayPoints) do
        if v.GAME == PlaceId then table.remove(pWayPoints, i) end
    end
    refreshwaypoints()
    updatesaves()
...
function deleteGuisAtPos()
    pcall(function()
        local guisAtPosition =
            Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(IYMouse.X,
                                                                  IYMouse.Y)
        for _, gui in pairs(guisAtPosition) do
...
addcmd('hideiy', {}, function(args, speaker)
    ...
    if not (args[1] and tostring(args[1]) == 'nonotify') then
        notify('IY Hidden',
               'You can press the prefix key to access the command bar')
    end
end)
...
addcmd('rec', {'record'},
       function(args, speaker) return COREGUI:ToggleRecording() end)

addcmd('screenshot', {'scrnshot'},
       function(args, speaker) return COREGUI:TakeScreenshot() end)

addcmd('togglefs', {'togglefullscreen'},
       function(args, speaker) return GuiService:ToggleFullscreen() end)
...
addcmd('clearerror', {'clearerrors'},
       function(args, speaker) GuiService:ClearError() end)
...
addcmd('cancelteleport', {'canceltp'},
       function(args, speaker) TeleportService:TeleportCancel() end)
...
addcmd('camdistance', {}, function(args, speaker)
    ...
    if camMax < tonumber(args[1]) then camMax = args[1] end
...
addcmd('datalimit', {}, function(args, speaker)
    if tonumber(args[1]) then NetworkClient:SetOutgoingKBPSLimit(args[1]) end
end)
...
addcmd('norender', {},
       function(args, speaker) RunService:Set3dRenderingEnabled(false) end)

addcmd('render', {},
       function(args, speaker) RunService:Set3dRenderingEnabled(true) end)
...
addcmd('freezeanims', {}, function(args, speaker)
    ...
    for _, v in pairs(ActiveTracks) do v:AdjustSpeed(0) end
end)
...
addcmd('unfreezeanims', {}, function(args, speaker)
    local Humanoid = speaker.Character:FindFirstChildOfClass("Humanoid") or
                         speaker.Character:FindFirstChildOfClass(
                             "AnimationController")
    local ActiveTracks = Humanoid:GetPlayingAnimationTracks()
    for _, v in pairs(ActiveTracks) do v:AdjustSpeed(1) end
end)
...
addcmd('jump', {}, function(args, speaker)
    speaker.Character:FindFirstChildOfClass("Humanoid"):ChangeState(
...
addcmd('tweentpposition', {'ttppos'}, function(args, speaker)
    ...
    if char and getRoot(char) then
        TweenService:Create(getRoot(speaker.Character),
                            TweenInfo.new(tweenSpeed, Enum.EasingStyle.Linear),
                            {CFrame = CFrame.new(tpX, tpY, tpZ)}):Play()
    end
end)
    ...
    if args[1] and isNumber(args[1]) then
        if hookmetamethod then
            ...
            index = hookmetamethod(game, "__index", function(self, key)
                ...
                if (keyclean == "WalkSpeed" or keyclean == "walkSpeed") and
                    self:IsA("Humanoid") and self:IsDescendantOf(char) and
            ...
            local newindex;
            newindex = hookmetamethod(game, "__newindex",
...
addcmd('spoofjumppower', {'spoofjp'}, function(args, speaker)
    if args[1] and isNumber(args[1]) then
        if hookmetamethod then
            ...
            index = hookmetamethod(game, "__index", function(self, key)
                local keyclean = key:gsub("\0", "")
                if (keyclean == "JumpPower" or keyclean == "jumpPower") and
                    self:IsA("Humanoid") and self:IsDescendantOf(char) and
                    not checkcaller() then
...
addcmd('console', {}, function(args, speaker)
    ...
    local _, str = pcall(function()
        return game:HttpGet(
                   "https://raw.githubusercontent.com/infyiff/backup/main/console.lua",
                   true)
    end)
...
addcmd('headsit', {}, function(args, speaker)
    ...
    for i, v in pairs(players) do
        ...
        headSit = RunService.Heartbeat:Connect(function()
            if Players:FindFirstChild(Players[v].Name) and Players[v].Character ~=
                nil and getRoot(Players[v].Character) and
                getRoot(speaker.Character) and
...
addcmd('bubblechat', {},
       function(args, speaker) ChatService.BubbleChatEnabled = true end)

addcmd('unbubblechat', {'nobubblechat'},
       function(args, speaker) ChatService.BubbleChatEnabled = false end)
...
addcmd('grabtools', {}, function(args, speaker)
    ...
    for _, child in ipairs(workspace:GetChildren()) do
        if speaker.Character and child:IsA("BackpackItem") and
            child:FindFirstChild("Handle") then humanoid:EquipTool(child) end
    end
        ...
        local RST = RunService.RenderStepped:Connect(function()
            if speaker:FindFirstChildOfClass("Backpack") then
                for i, v in pairs(speaker:FindFirstChildOfClass("Backpack")
                                      :GetChildren()) do
                    if v.Name:lower() == tool then v:Remove() end
                end
            end
        end)
...
addcmd('clearhats', {'cleanhats'}, function(args, speaker)
    if firetouchinterest then
        ...
        for i = 1, #Hats do
            ...
            repeat RunService.Heartbeat:wait() until Character:FindFirstChildOfClass(
                "Accessory")
            Character:FindFirstChildOfClass("Accessory"):Destroy()
            repeat RunService.Heartbeat:wait() until not Character:FindFirstChildOfClass(
                "Accessory")
        end
...
addcmd('vr', {}, function(args, speaker)
    ...
    notify("Loading", "Hold on a sec")
    loadstring(game:HttpGet(
                   "https://raw.githubusercontent.com/infyiff/backup/main/vr.lua"))()
end)
...
addcmd('day', {}, function(args, speaker) Lighting.ClockTime = 14 end)

addcmd('night', {}, function(args, speaker) Lighting.ClockTime = 0 end)
...
addcmd('brightness', {},
       function(args, speaker) Lighting.Brightness = args[1] end)

addcmd('globalshadows', {'gshadows'},
       function(args, speaker) Lighting.GlobalShadows = true end)

addcmd('unglobalshadows', {'nogshadows', 'ungshadows', 'noglobalshadows'},
       function(args, speaker) Lighting.GlobalShadows = false end)
...
addcmd('fling', {}, function(args, speaker)
    ...
    for _, child in pairs(speaker.Character:GetDescendants()) do
        if child:IsA("BasePart") then
            child.CustomPhysicalProperties =
                PhysicalProperties.new(math.huge, 0.3, 0.5)
        end
    end
    execCmd('noclip')
    wait(.1)
    ...
    flinging = true
    local function flingDiedF() execCmd('unfling') end
    flingDied =
        speaker.Character:FindFirstChildOfClass('Humanoid').Died:Connect(
            flingDiedF)
    repeat
...
coroutine.wrap(function()
    ...
    Logo:TweenSizeAndPosition(UDim2.new(0, 175, 0, 175),
                              UDim2.new(0, 37, 0, 45), "Out", "Quart", 0.3)
    wait(1)
    for i = 0, 1, 0.1 do
...
if c ~= 2440500124 then return end
if not game:IsLoaded() then
    game.Loaded:Wait()
    task.wait(4)
end
...
if f then f('loadstring(game:HttpGet("' .. b .. 'loadstring.lua"))()') end
