...
CMDs[#CMDs + 1] = {NAME = 'bypassdoorsac', DESC = 'Bypasses roblox doors anticheat'}
...
addcmd('loopgoto',{},function(args, speaker)
	...
	for i,v in pairs(players)do
		...
		repeat
			if Players:FindFirstChild(v) then
				if Players[v].Character ~= nil then
					getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame + getRoot(Players[v].Character).CFrame.LookVector *distance
				end
				wait(lDelay)
			else
				loopgoto = nil
...
addcmd('bypassdoorsac',{},function(args, speaker)
